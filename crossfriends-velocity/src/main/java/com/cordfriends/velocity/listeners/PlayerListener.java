    ...
    private void captureSkin(Player player, PlayerProfile profile) {
        for (GameProfile.Property property : player.getGameProfileProperties()) {
            if ("textures".equals(property.getName())) {
                ...
                plugin.getLogger().info("Skin capture et mis en cache pour " + player.getUsername()
                        + " (" + player.getUniqueId() + ").");
                return;
            }
        }
        ...
        plugin.getLogger().warn("Aucune propriete \"textures\" recue pour " + player.getUsername()
                + " (" + player.getUniqueId() + ") a la connexion : son GameProfile n'en contient pas "
                + "(compte hors-ligne sans systeme de skin, ou forwarding proxy->auth mal configure).");
    }
}
